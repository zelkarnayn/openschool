...
import {ArticleCardType, getArticleCards} from "../../../../entities/article/model";

function ArticlesPagination() {
    const data = useSelector((state) => state.articles.posts)
    const dispatch = useDispatch()
    const [skip, setSkip] = useState(0)
    
    const handleNextArticles = () => {
        if (data.skip < data.total && data.limit === 12) {
    ...
    return (
        <div className={styles.articles__wrapper}>
            ...
            <div className={styles.articles__cards}>
                {data.posts?.map((post: ArticleCardType) => <ArticleCard key={post.id} title={post.title} body={post.body} tags={post.tags} reactions={post.reactions} id={post.id}/>)}
            </div>
            <div className={styles.articles__pagination}>
                <svg onClick={() => handlePreviousArticles()} style={data.skip >= data.limit ? {fill: '#605E5E'} : {fill: '#CECECE'}} width="46" height="30" viewBox="0 0 46 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <svg onClick={() => handleNextArticles()} style={(data.skip < data.total && data.limit === 12) ? {fill: '#605E5E'} : {fill: '#CECECE'}} width="46" height="30" viewBox="0 0 46 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fillRule="evenodd" clipRule="evenodd"
